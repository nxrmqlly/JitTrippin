...
func (ro *Router) routes() {
	...
	ro.Handle("DELETE /api/v1/auth/{provider}", ro.handleAuthUnlink, ro.Authentication)
	ro.Handle("GET    /api/v1/auth/me", ro.handleAuthMe, ro.Authentication)

	ro.Handle("GET    /api/v1/integrations/providers", ro.handleIntegrationProviders)
	ro.Handle("GET    /api/v1/integrations", ro.handleIntegrationList, ro.Authentication)
	ro.Handle("POST   /api/v1/integrations/{provider}", ro.handleIntegrationAdd, ro.Authentication)
	ro.Handle("DELETE /api/v1/integrations/{provider}", ro.handleIntegrationRemove, ro.Authentication)

	ro.Handle("POST   /api/v1/integrations/github/webhook", ro.handleGithubWebhook)
	ro.Handle("POST   /api/v1/integrations/github/repos", ro.handleGithubAdd, ro.Authentication)
	ro.Handle("GET    /api/v1/integrations/github/list", ro.handleGithubList, ro.Authentication)
	ro.Handle("DELETE /api/v1/integrations/github/repos/{id}", ro.handleGithubRemove, ro.Authentication)
...
func runError(w http.ResponseWriter, err error) {
	...
	default:
		...
		httpx.InternalServerError(w, err) // 500
	}
}
